    ...
    <container>
        ...
        <el-form :inline="true" :model="query" ref="query" class="goods_query_form">
            ...
            <el-form-item prop="categoryId">
                <el-cascader v-model="query.categoryId" :options="categoryOption" @focus="getCategoryOption"
            ...
            <el-table-column label="操作" min-width="130">
                <template #default="scope">
                    ...
                    <el-button v-if="scope.row.status === 2" size="small" type="primary"
                        @click="changeStatus(1, scope.row)">上架
                    </el-button>
                    <el-button v-if="scope.row.status === 1" size="small" type="primary"
                        @click="changeStatus(2, scope.row)">下架
                    </el-button>
                    <el-popconfirm title="此操作将永久删除该信息, 是否继续?" confirmButtonText="确认" cancelButtonText="取消"
                        cancelButtonType="default" :icon="WarningFilled" @confirm="deleteGoods(scope.row)">
                        <template #reference>
        ...
        <div style="padding: 15px 0;">
            <el-pagination layout="total, prev, pager, next" :current-page="pageNum" :page-size="pageSize"
                :total="total" @current-change="handleCurrentChange" @prev-click="handleCurrentChangePrev"
                @next-click="handleCurrentChangeNext" background />
        </div>
        <!-- 添加、编辑商品，通用对话框 -->
        <el-dialog :title="dialogTitle" v-model="goodsDialogVisible" :lock-scroll="false" top="5vh" width="45%"
            @close="cancel">
            <el-form ref="ruleForm" :rules="rules" :model="goods" label-width="80px">
                <el-form-item label="类目" prop="categoryId">
                    <el-cascader v-model="goods.categoryId" :options="categoryOption" @change="changeCategory"
                        change-on-select @focus="getCategoryOption" placeholder="请选择" clearable />
                </el-form-item>
                ...
                <el-form-item label="图片" prop="imageUrl">
                    ...
                    <el-upload action="http://localhost:8000/web/upload" :headers="{ 'token': token }" :limit="1"
                        name="file" :file-list="pictureList" list-type="picture-card" :on-preview="handleImagePreview"
                        :on-remove="handleImageRemove" :on-success="handleImageSuccess">
                        <div class="goods_image_upload_icon">+</div>
                    </el-upload>
                </el-form-item>
                <el-form-item label="备注">
                    <el-input v-model="goods.remark" style="width: 90%;" type="textarea" maxlength="100"
